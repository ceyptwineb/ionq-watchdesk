...
async function readPostedIds() {
  try {
    ...
    // 新方式: posted/<id> の個別キー
    try {
      const result = await store.list({ prefix: "posted/" });
      ((result && result.blobs) || []).forEach((entry) => {
        ids.push(decodeURIComponent(String(entry.key).slice("posted/".length)));
      });
    } catch (e) { /* list未対応環境なら旧方式のみ */ }
    // 旧方式: posted-state に配列(移行前の残骸があれば拾う)
    try {
